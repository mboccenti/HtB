...
def manageArgs():
    ...
    auth.add_argument('-A', dest='authentication', type=str, default="", help='Authentication')
...
def downloadSha(url, port, docker, sha256, authentication=None):
    createDir(docker)
    directory = f"./{docker}/"
    for sha in sha256:
...
def dump(args):
    ...
    downloadSha(args.url, args.port, args.dump, sha256, args.authentication)

def dumpAll(args):
    # dockerlist = enumList(args.url, args.port, args.username,args.password)
    # dockerlist = enumList(args.url, args.port, args.authentication)
    ...
    for docker in dockerlist[1:]:
        ...
        downloadSha(args.url, args.port,docker,sha256,args.authentication)
